...
function getLatestInfo() {
    try {
        marriageContract = web3.eth.contract(JSON.parse(document.getElementById("ABI").value)).at(document.getElementById("contract").value);
...
function checkOnHudsonsMarriage() {
    try {
        ...
        var marriageDateRaw = marriageContract.marriageDate({ from: web3.eth.coinbase });
        var marriageDate = moment(parseInt(marriageDateRaw, 10) * 1000).format('MMMM Do YYYY, h:mm:ss a');
        console.log(marriageDate);

        document.getElementById("mStatus").value = web3.toAscii(marriageContract.marriageStatus({ from: web3.eth.coinbase }));;
        document.getElementById("partner1").value = web3.toAscii(marriageContract.partner1({ from: web3.eth.coinbase }));;
        document.getElementById("partner2").value = web3.toAscii(marriageContract.partner2({ from: web3.eth.coinbase }));;
        document.getElementById("mDate").value = marriageDate;
        marriageImageHash = web3.toAscii(marriageContract.imageHash({ from: web3.eth.coinbase }));
        document.getElementById("mImage").innerHTML = "<a href=https://ipfs.pics/" + marriageImageHash + " &quot; target=&quot;_blank&quot;>" + marriageImageHash + "</a>";
        marriageProofHash = web3.toAscii(marriageContract.marriageProofDoc({ from: web3.eth.coinbase }));;
...
function getMajorEvents() {
	...
	var event = marriageContract.MajorEvent( {}, {fromBlock: 0, toBlock: 'latest'});
